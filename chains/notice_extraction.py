...
class NoticeEmailExtract(BaseModel):
    ...
    required_changes: str | None = Field(
        default=None,
        description="The required changes (if present in the message)"
    )
    ...
    compliance_deadline_str: str | None = Field(
        ...
        exclude=True,
        repr=False,
        description="The date that company must comply (if any) reformatted to match YYYY-mm-dd"
    )

    max_potential_fine: str | None = Field(
        default=None,
        description="The maximum potential fine (if any)"
    )

    @staticmethod
    def _conver_string_to_date(date_str: str | None) -> date | None:
        try:
    ...
    def date_of_notice(self) -> date | None:
        return self._conver_string_to_date(self.date_of_notice_str)

    @computed_field
    @property
    def compliance_deadline(self) -> date | None:
        return self._conver_string_to_date(self.compliance_deadline_str)
...
notice_parse_model = ChatOpenAI(model="gpt-4o-mini", temperature=0)

NOTICE_PARSER_CHAIN = (
    info_parse_prompt |
    notice_parse_model.with_structured_output(NoticeEmailExtract)
)